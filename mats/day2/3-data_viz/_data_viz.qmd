...
df_lifetime <- readr::read_csv(here::here("data/tidy_data_lifetime_pilot.csv"), 
  ...
  filter(type == "critical", # only critical trials
         px != "px7") # this participant had lots of 0's for some reason
```
...
2. create an object with some summary statistics
...
df_lifetime <- df_lifetime %>%
  mutate(region = factor(region, levels = c("verb-1","verb","verb+1","verb+2","verb+3","verb+4")))
...
```{r, out.width="100", fig.pos="center", echo = F}
#| output-location: fragment
...
- histograms and density plots plot a **single variable** along the x-axis
  + in most other plots the dependent (measure) variable is plotted along the y-axis
- scatterplots plots the relationship between **two variables**
