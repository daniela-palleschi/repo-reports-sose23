...
Model summaries are provided in @tbl-fp-inter.
...
```{r}
broom.mixed::tidy(fit_tt_mixed, effects = "ran_pars") %>% 
  kable() %>% 
  kable_styling()
```

```{r}
broom.mixed::tidy(fit_tt_mixed, effects = "ran_vals") %>% 
  filter(group == "px") %>% 
...
# Plot model coefficients (`sjPlot` package)

## Fixed effects

```{r}
sjPlot::plot_model(fit_tt_mixed) +
  geom_hline(yintercept=0, linetype = "dashed", color = "grey") +
  labs(title = "Slope estimates for log(tt)") +
  theme_bw()
```


```{r}
sjPlot::plot_model(fit_tt_mixed, type = "int") +
  labs(title = "Fitted values per condition") +
  geom_hline(yintercept=0, linetype = "dashed", color = "grey") +
